...
def buscar_jogos_ao_vivo(timeout: int = DEFAULT_TIMEOUT) -> List[Dict[str, Any]]:
    """Busca jogos ao vivo da Copa do Mundo, com fallback para o endpoint geral."""
    jogos: List[Dict[str, Any]] = []
    tentativas: List[Tuple[str, Optional[Dict[str, Any]]]] = [
        ("live league=27", {"league": WORLD_CUP_LEAGUE_ID}),
        ("live geral", None),
    ]
    ...
    for rotulo, params in tentativas:
        try:
            ...
            print(f"[BSD] Falha ao consultar {rotulo}: {exc}")
            continue

        jogos = [_normalizar_evento_ao_vivo(evento) for evento in eventos if _evento_eh_copa_ao_vivo(evento)]
        print(f"[BSD] {len(jogos)} jogos ao vivo da Copa encontrados em {rotulo}.")
        if jogos:
            return jogos

    print("[BSD] Nenhum jogo ao vivo da Copa encontrado.")
    return jogos
